...
module.exports = 
{
	...
	init: function (param)
	{
		...
		entityos.add(
		{
			...
			code: function (param)
			{
				app.invoke('ai-gen-util-vector-store-link-file', param);
			}
		});

		entityos.add(
		{
			name: 'ai-gen-util-vector-store-link-file',
			code: function (param)
			{
				const event = entityos.get({scope: '_event'});
		...
		entityos.add(
		{
			name: 'ai-gen-util-vector-store-unlink-file',
			code: function (param)
			{
				const event = entityos.get({scope: '_event'});
				...
				openai.vectorStores.files.del(vectorStore.id, file.id)
				.then(function (vsFile)
				{				
					_.set(param, 'vectorstorefile.id', vsFile.id);
				...
				.catch(function(err)
				{
					...
					entityos.invoke('util-end', {data:
					{
						...
						file: {error: 'Error unlinking file from vector store'}
					}});
				});
			}
		});
		...
		entityos.add(
		{
			...
			code: function (param)
			{
				...
				if (file.base64 != undefined)
				{
					...
					file._pathFileName = path.join('data/_temp/', file.name); // __dirname
		...
		entityos.add(
		{
			...
			code: function (param)
			{
				...
				.catch(function(err)
				{
					...
					entityos.invoke('util-end', {data:
					{
						file: {error: 'Error removing file'}
					}});
				});
			}
		});
